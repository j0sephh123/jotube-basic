...
export default function Footer({ slot }: { slot: React.ReactNode }) {
  const { handlePrevious, handleNext } = usePaginate();
  const episodesCount = useEpisodesCount();
  const { selectedItems: selectedImages, currentIndex } =
    useEpisodesProcessingState();
  const isLastItem = useIsLastItem();
  const isPreviousDisabled = episodesCount === 0 ? false : currentIndex === 0;
