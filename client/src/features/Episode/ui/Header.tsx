import { SmoothProgressBar } from "@features/Thumbnails";
import { useEpisodesCount } from "@features/Episode";
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2">
          <CustomLink to={channelLink}>
            <div className="ml-4 text-gray-300 text-sm">TV: {title ?? ""}</div>
          </CustomLink>
          <div className="text-gray-300 text-sm">{videoLabel}</div>
          <div className="text-gray-300 text-sm">
            {currentIndex + 1} / {episodesCount}
          </div>
        </div>
      </div>
